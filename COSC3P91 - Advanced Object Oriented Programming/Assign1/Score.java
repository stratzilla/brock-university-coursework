...
public class Score {
    ...
    private final int theScore; // the player's score
    ...
    Score(Hand h) {
        playerHand = h;
        getFrequencies(); // populate histograms
    ...
    private int tabScore() {
         ...
         */
        if (checkRoyalFlush()) { return 10; } // Royal Flush
        if (checkStraightFlush()) { return 9; } // Straight Flush
        if (checkMult(4)) { return 8; } // Four of a Kind
        if (checkFullHouse()) { return 7; } // Full House
        if (checkFlush()) { return 6; } // Flush
        if (checkStraight()) { return 5; } // Straight
        if (checkMult(3)) { return 4; } // Three of a Kind
        if (checkTwoPair()) { return 3; } // Two Pairs
        if (checkMult(2)) { return 2; } // One Pair
        return 1; // High Card
    }
    
    /**
     * populates an array of frequencies of ranks and suits like a histogram
     */
    ...
    private boolean checkStraight() {
        ...
        // to count the number of successive ranks
