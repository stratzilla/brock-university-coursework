...
// define my own min and max functions
...
/**
 * I wanted to use classes for Pixel and Image
 * but for some reason the Display Function only really wants
 * a stream of GLubyte
 *
 * I guess I could have got away with Image being a class with
 * a getter function for *Pixel but I realized it would clutter
 * things a bit as direct access to struct attributes is better
 * than calling getter/setter member functions in a class
 *
 * Perhaps for future assignments I can figure out a good middleground
 */
...
* Quite literally taken from solution framework provided
*
...
int main(int argc, char** argv) {
	saveBuffer = imageLoader("image.tif"); // load save buffer
