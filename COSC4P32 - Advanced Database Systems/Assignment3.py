...
def fetchNext(self): # AggregatorExecutor
	...
	if not self.group_map: # if the map has not yet been added to
		...
		# get corresponding indices in schema
		group_idx = self.child_executor.plan.getOutputSchema().getColunmIdx(group_by)
		agg_idx = self.child_executor.plan.getOutputSchema().getColunmIdx(agg_at)
		curr = self.child_executor.fetchNext() # keep track of currently iterated tuple
		while curr is not None: # while not EOF
			# add to map
			self.insert(curr[group_idx], curr[agg_idx])
			curr = self.child_executor.fetchNext() # go to next
	# since you can't tell if a generator is empty, need a try/catch to handle error
	try:
		key = next(self.fetchGen) # get the next yielded row from generator
	except StopIteration: # if null
		return None # you've reached EOF
	agg_type = self.plan.getAggregateType() # find out what exactly to return
	# and set the value to whatever that value is to return
	if agg_type == "MIN":
		value = self.group_map[key].getMin()
	elif agg_type == "MAX":
		value = self.group_map[key].getMax()
	elif agg_type == "SUM":
		value = self.group_map[key].getSum()
	elif agg_type == "AVG":
		value = self.group_map[key].getAvg()
	elif agg_type == "COUNT":
		value = self.group_map[key].getCount()
	return (key, value) # otherwise return the row
