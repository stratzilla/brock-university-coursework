...
class Particle:
	...
	def set_pos(self, pos):
		...
		# if particle is still bounded
		if not any(p < -BOUND for p in pos)\
		and not any(p > BOUND for p in pos):
			# set new particle position
			self.pos = pos
		# if particle is unbounded
		else:
			# randomize particle within search space
			self.pos = [random.uniform(-BOUND, BOUND) for _ in range(len(pos))]
...
if __name__ == '__main__':
	...
	SWARM_SIZE = par.get_swarm_size(int(argv[3]))
	EPOCHS = par.get_epochs()
	W, C_1, C_2, BOUND = par.get_pso_params(int(argv[4]))
	if not AUTO:
		print("\nParameters:")
		print(f"Network topology: {FEATURES}-{HIDDEN_SIZE}-{CLASSES}")
		print(f"Dimensionality: {DIMENSIONS}")
		print(f"Inertial Coefficient: {W}")
		print(f"Cognitive Coefficient: {C_1}")
		print(f"Social Coefficient: {C_2}")
